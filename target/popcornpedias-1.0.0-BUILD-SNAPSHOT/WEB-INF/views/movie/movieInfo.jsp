    ...
    <div class="container">
        <div class="movieInfoBox">
            <div class="d-flex align-items-center">
                ...
                <div class="movieLike">
                    <button type="button" class="btn btn-outline-light rounded-circle btn-sm"
                            onclick="wishMoviebtn('${dto.movie_id }','${user.member_id}')" id="wishMoviebtn_">
                        <i class="fas fa-heart"></i>
                        <span></span>
                    </button>
                </div>
            </div>
            <c:if test="${not empty dto.movieNm_EN }">${dto.movieNm_EN }
                <div class="vr align-middle m-1"></div>
            </c:if>
            <c:if test="${not empty dto.movieNation }">${dto.movieNation }
                <div class="vr align-middle m-1"></div>
            </c:if>
            <c:if test="${not empty dto.movieGenre }">${dto.movieGenre }
                <div class="vr align-middle m-1"></div>
            </c:if>
            ...
            <div>
                ...
                <div id="starRatingContainer" class="toCenter text-center parent">
					...
					<span class="star align-middle">
				  		★★★★★
					  	<span>★★★★★</span>
						<input type="number" id="starRating" name="starRating" oninput="drawStar(this)" value="0"
                               step="0.5" min="0" max="5" onclick="starRatingOnClick(event)"/>
						<input type="hidden" id="updateStarRatingForm">
				  	</span>
                    </form>
                    <button id="showDetailRatingButton" class="btn btn-primary"
                    ...
                    <div id="detailRatingForm" class="RatingForm">
                        ...
                        <button id="deleteDetailRatingButton" onclick="deleteDetailStarRating()" class="detailButton">
                            삭제하기
                        </button>
            ...
            <div class="text-center toCenter member-review-box mb-3">
                ...
                <div id="reviewButton">
                    ...
                    <c:if test="${not empty user.member_id }">
                        <a class="writeReview btn btn-light" href="javascript:void(0)" id="writeReviewButton">클릭하여 리뷰를
                            작성해주세요 </a>
                    </c:if>
...
<div class="writeReviewModal">
    <div class="modalContent">
        <form id="commentForm" action="${contextPath}/movie/writeReview.do" method="post">
            <textarea class="modal_repCon" name="content" id="myCommentContent" required="required" maxlength="300"
                      placeholder="최대 300자까지 작성가능합니다."></textarea>
...
<div class="modReviewModal">
    <div class="modalContent">
        <form id="reviewForm" action="${contextPath}/movie/modReview.do" method="post">
            <textarea class="modal_repCon" name="content" id="myReviewContent" required="required"
    ...
    function writeDetailStarRating() {
        ...
        $.ajax({
            ...
            data: {
                ...
                'rating': rating,
                'detailOST': OSTRating,
                'detailDirection': directionRating,
                'detailVisual': visualRating,
                'detailActing': actingRating,
                'detailStory': storyRating
            },
            success: function (result) {
                alert("반영 완료");
            }
