            ...
            <table class="add-form-box">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" onclick="fn_getKobisAPI()" class="btn btn-primary" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .85rem;">Kobis 정보 가져오기 (영화이름+감독명 필수)</button>
                    </td>
                </tr>
                <tr>
                    <th>영화명(영문)</th>
                ...
                <tr>
                    <th>TMDB 자료 가져오기</th>
                    <td>
                        ▶<a href="https://www.themoviedb.org/search?language=ko&query=${movieDTO.movieNm }" target="_blank"><b>TMDB로 이동하기(영화검색 후 번호 복사)</b></a><br>
                        TMDB movieID <input type="text" id="moviecode" placeholder="숫자만 입력" size="10">
                        <button type="button" onclick="fn_getURL()" class="btn btn-secondary" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;">URL 가져오기</button>
                    </td>
                </tr>
                <tr>
                    <th>영화 포스터 URL(자동입력)</th>
                    <td><input type="text" size="80" id="posterURL" readonly style="background-color:#eee;" placeholder="poster url" name="moviePosterPath" value="${movieDTO.moviePosterPath }"></td>
                </tr>
                <tr>
                    <th>영화 세부정보용 배경 URL(자동입력)</th>
                    <td><input type="text" size="80" id="backdropURL" readonly style="background-color:#eee;" placeholder="backdrop url" name="movieBackdropPath" value="${movieDTO.movieBackdropPath }"></td>
                </tr>
                <tr>
                    <th>영화 줄거리(자동입력)</th>
                    <td><input type="text" size="80" id="overview" readonly style="background-color:#eee;" placeholder="줄거리" name="movieOverview" value="${movieDTO.movieOverview }"></td>
                </tr>
            </table>
        </div>
		<div style="text-align:center; margin:20px auto 70px;" >
		    <input type="submit" class="btn btn-primary" style="width:200px;" value="<c:if test="${empty movieDTO  }">추가하기</c:if><c:if test="${not empty movieDTO }">수정하기</c:if>">
		</div>
	</form>
</div>
</div>
