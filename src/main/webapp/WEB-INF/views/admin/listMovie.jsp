...
<form method="get" action="${contextPath}/admin/searchMovie.do" align="center">
    ...
    <div style="display: flex; justify-content: space-between;">
        ...
        <div><a href="/popcornpedias/admin/movieForm" class="btn btn-primary mb-1">영화 추가하기</a></div>
    </div>
...
<td class="border"><a href="${contextPath}/admin/updateMovieForm.do?movie_id=${movie.movie_id}"
                      class="btn btn-secondary"
                      style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .85rem;">수정</a><br><br>
    <a href="${contextPath}/admin/deleteMovie.do?movie_id=${movie.movie_id}" class="btn btn-danger"
       onclick="confirmDelete(event)"
       style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .85rem;">삭제</a></td>
</tr>
</c:forEach>
</table>
</div>
...
<div class="board-pagination-container mt-4 d-flex justify-content-center">
    <ul class="pagination justify-content-center paging">
        ...
        <c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
            <li class="page-item">
                <a class="page-link <c:if test='${selectPageNum == num}'>active</c:if>"
                   href="${pageUrl}${num}${pageQuery}">${num}</a>
            </li>
        </c:forEach>
        <c:if test="${page.next}">
            <li class="page-item">
